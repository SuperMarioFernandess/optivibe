...
def test_cylinder_eta_is_bit_identical_through_layer(variant_a: VariantConfig) -> None:
    """eta on fixed inputs is unchanged after the refactor onto the layer."""
    ...
    assert eta.tolist() == list(_CYL_ETA_ANCHOR)  # exact, no tolerance
    assert model.eta_working_point() == _CYL_ETA0_ANCHOR
